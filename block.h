	...
	Type *New(int num = 1)
	{
		...
		if (!last || last->current + num > last->last)
		{
			...
			else
			{
				...
				if (!next) { if (error_function) (*error_function)("-block-126- Not enough memory!"); exit(1); }
...
public:
	...
	Type *New()
	{
		...
		if (!first_free)
		{
			...
			if (!first) { if (error_function) (*error_function)("-block-252- Not enough memory!"); exit(1); }
